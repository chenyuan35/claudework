...
### 章节连贯性审计（SOP-AUDIT-01，写完后必须跑）

每次正文写完后、质量门前，逐项自检。任一项 FAIL 则退回重写该区块，禁止补丁。

| # | 检项 | PASS 条件 | FAIL 示例 |
|:-|:-----|:----------|:----------|
| 1 | 首段衔接 | 第一句话自然引入首 h2 主题，不自说自话 | "如果你读到了这里"、"每个人都有" 等万能开头 |
| 2 | h2 独立性 | 6 个 h2 各承载独立子观点，不重复不交叉 | h2#3 和 h2#5 都在讲"朋友变淡" |
| 3 | 尾段收束 | 尾段是全文自然结束，非硬凑的万能段落 | "游戏会过时但友谊不会"（与上行无逻辑连接） |
| 4 | 无补丁段 | 每段与前后段有逻辑连接，无法独立抽离 | 任意一段删除后上下文仍通顺 → 该段是补丁 |
| 5 | 段落不串题 | 每段只讨论所在 h2 的子主题 | h2#4 段落中出现"这个问题最有争议"（属 h2#3） |
| 6 | 无 AI 套话 | 全文无"值得一提的是""在当今社会""综上所述"等模板腔 | |
| 7 | 无递归开头 | h2 标题不重复出现在段落首句 | h2="开黑" → 段落首句="开黑是一种..." |

**执行方式**：通读全文一遍，以上 7 项逐项标记 PASS/FAIL。全部 PASS 才进质量门。任一项 FAIL → 整段重写，不准局部修补。
