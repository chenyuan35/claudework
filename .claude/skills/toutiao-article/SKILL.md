...
🚨 字数硬性规则（不可违反）：正文汉字≥1500，不设上限。每段汉字≤80。底线是熔断判据不是写作目标。写前先做段落预算（33-39段×段均≈1500），算不够不落笔。核验必须用 `text.match(/[一-鿿㐀-䶿豈-﫿]/g)` 计汉字，禁止用 `len(str)` 或 `charCount` 代替。写完后汉字不足→整篇删掉重写，重新做段落预算后再落笔。
...
|---|---|---|---|
| 开头钩子(1-2) | 2 | 40 | 80 |
| 方法1 主体 | 5-6 | 50 | 250-300 |
| 方法2 主体 | 5-6 | 50 | 250-300 |
| 方法3 主体 | 6-7 | 50 | 300-350 |
| 呼吸短段 | 5-7 | 20 | 100-140 |
| 中段钩子 | 2 | 20 | 40 |
| 对比总结 | 3-4 | 45 | 135-180 |
| 结尾收束 | 2 | 40 | 80 |
...
全文段落总数 25-35 段（含配图/标题/呼吸段/中段钩子）。
「呼吸点」指：短段≤25字 / 金句单独段 / 单句改问段。每3段纯文字之后必须放1个呼吸点。
「中段钩子」指：在文章约300字和600字处各埋一个互动钩子（反问/悬念/预告）。
「段」指 <p>...</p>。
每段不超过80汉字（手机端约4行，超过4行=划走）。
...
【段3-8】方法主体（3个方法，每个方法4-6段）
- 每个方法按【观点句1句→操作步骤2-3句→案例1-2句→效果/数据1句】展开。
- 每个方法必须：
  - 至少有1个具体案例（"我一个朋友""我一个同事"）+ 具体数字
  - 方法末尾用口语化短句过渡到下一个（"但下面这个更省事"）
- 第1个方法结束后（约300字处）插入【中段钩子1】。
- 第2个方法结束后（约600字处）插入【中段钩子2】。

【段9-10】对比总结（用表格或对比句呈现）
- 方法优劣对比 / 使用前后对比 / 花费对比
- 让读者一眼看到"用这个方法值不值"

【段11-12】结尾（4种之一，连续两篇不同类型）
...
读者在手机上看了3段纯文字后会产生视觉疲劳。每3-4段纯文字之后，插入配图、短段（≤30字）或金句单独成段作为呼吸点。
...
| 正文结尾缺少呼吸感 | 连续3段纯文字 > 130字无短段穿插 | 在结尾部分每 2 段后插入 ≤30 字的呼吸短段（如"每个人家里都有那么一堆'留着没用、扔了可惜'的东西吧？"），不让长段堆砌在最后 |
