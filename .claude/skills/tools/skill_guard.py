...
def check_browser_publish(text: str) -> list:
    checks = []
    h = bool(re.search(r'选择器表|坐标速查|坐标表|选择器.*坐标|用途.*坐标|用途.*选择器', text, re.IGNORECASE))
    ...
    h = bool(re.search(r'pass.*true.*才|验证.*通过.*才|不通过禁止|通过.*后.*保存|确认.*后.*发布', text, re.IGNORECASE))
